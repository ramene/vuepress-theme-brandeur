<template>
    <div class="pf lg:w-1/2 md:w-1/2 sm:w-full lg:flex-none bg-transparent" 
                tabindex="0" 
                @click="updateActivePlan" 
                @keydown.space="updateActivePlan">
      <div class="flex mb-6 justify-between items-center ">
        <h1 class="st text-2xl sm:text-xl md:text-2xl lg:text-2xl xl:text-2xl">
          {{ this.planName }}
          <br class="inline"><span class="  text-white">{{ this.planSubtitle }}</span>
        </h1>
      ...
      <div class="mt-4 sm:mt-6">
        ...
        >
          <NavLink
            class="btn btn-indigo bg-brand-ice shadow-lg sm:text-base"
            :item="actionLink"
          />
        </p>
        <!-- <a href="#" class="btn bg-brand-ice shadow-lg sm:text-base">Contact Sales</a> -->
      </div>
    </div>
</template>
...
export default {
  ...
  props: ['planName', 'planSubtitle', 'planGigs', 'planDollars', 'activePlan'],
  methods: {
    updateActivePlan() {
